...
async function printMarkdown(content: string): Promise<void> {
    if (process.stdout.isTTY) {
        // Lazy load marked and marked-terminal
        const { marked } = await import("marked");
        const { markedTerminal } = await import("marked-terminal");

        // marked-terminalを拡張として登録
        // Note: marked is a singleton, but since CLI runs once, it's fine.
        // We could guard this but in this context it's unnecessary overhead.
